...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("levitate"))
        return false;
...
{
    // Convert levitate:// to levitate:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("levitate://"))
    {
        uri.replace(0, 12, "levitate:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Levitate.lnk";
}
...
{
    return GetAutostartDir() / "levitate.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Levitate-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  levitate-qt [" + tr("command-line options") + "]                     " + "\n";
